...
export const publicAppOrigin = import.meta.env.VITE_PUBLIC_APP_URL?.trim() || "https://terra-trus-t-ai-prototype.vercel.app";
  ...
  return (
    <section className="surface-card overflow-hidden border-primary/15">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border p-5">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">TerraTrust AI</p>
          <h3 className="mt-1 font-display text-xl font-semibold">Digital Property Passport</h3>
          <p className="mt-1 text-sm text-muted-foreground">Scan to verify this property passport</p>
        </div>
      ...
      <div className="grid gap-5 p-5 sm:grid-cols-[auto_1fr] sm:items-center">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 break-all font-mono text-sm font-semibold text-foreground">{property.passportId}</p>
          <p className="mt-3 break-all text-xs text-muted-foreground">{url}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Button size="sm" variant="outline" onClick={downloadSvg}><Download className="mr-1.5 h-4 w-4" /> Download SVG</Button>
            <Button size="sm" variant="outline" onClick={copyLink}>{copied ? <Check className="mr-1.5 h-4 w-4" /> : <Copy className="mr-1.5 h-4 w-4" />} {copied ? "Copied" : "Copy link"}</Button>
            <Button size="sm" variant="ghost" asChild><a href={url}><ExternalLink className="mr-1.5 h-4 w-4" /> Open passport</a></Button>
            <Button size="sm" variant="ghost" asChild><Link to="/qr-generator">Create custom QR</Link></Button>
          </div>
        </div>
      </div>
    </section>
