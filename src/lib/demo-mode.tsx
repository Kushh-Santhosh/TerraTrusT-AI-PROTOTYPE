...
const STEPS: DemoStep[] = [
  {
    ...
    body: "Every property in one portfolio. KPIs roll up from real registry signals — no spreadsheets, no missing fields.",
  },
  {
    to: "/properties/p_001",
    title: "Property Passport",
    body: "The machine-verifiable identity of a parcel: trust score, AI valuation, ownership history, encumbrances, nearby infrastructure.",
  },
  {
    to: "/ai-confidence",
    title: "Confidence Engine",
    body: "Eight weighted signals — government docs, community, surveyor, GIS, utilities, tax, fraud, ownership — fused into one explainable score.",
  },
  {
    to: "/ai-valuation",
    title: "Explainable AI valuation",
    body: "Every rupee of the estimate is traceable to a comparable sale, infrastructure feature, or market factor.",
  },
  {
    to: "/ai-fraud",
    title: "Fraud detection",
    body: "Duplicate boundaries, forged stamps, signature anomalies — surfaced before they reach the bureau desk.",
  },
  {
    to: "/government",
    title: "Government workbench",
    body: "Officers review AI suggestions, approve or escalate, and issue the digital passport in minutes.",
  },
  {
    to: "/analytics",
...
export function DemoModeProvider({ children }: { children: ReactNode }) {
  ...
  const value: Ctx = useMemo(
    () => ({
      ...
      step,
      total: STEPS.length,
      current: active ? STEPS[step] : null,
      start,
      stop,
      next,
      prev,
...
export function StartDemoButton({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={
        className ??
        "inline-flex items-center gap-2 rounded-full bg-foreground px-5 py-2.5 text-sm font-medium text-background hover:opacity-90"
      }
    >
