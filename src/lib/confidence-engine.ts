...
export function computeConfidence(p: Property): ConfidenceReport {
  ...
  const rows: Array<[keyof typeof WEIGHTS, string, number, string, string[]]> = [
    ...
    [
      ...
      Object.keys(p.sourceChecks ?? {}).map(
        (source) => `${source}: ${String(p.sourceChecks?.[source])}`,
      ),
    ],
    [
      "surveyor",
      ...
      [
        p.boundary.length ? "RTK GPS capture · ±0.4m" : "Sketch only",
        "Inspection photos · 12 frames",
        "Surveyor digital signature",
      ],
    ],
    [
      "gisBoundary",
      "GIS boundary accuracy",
      gisBoundary,
    ...
    [
      ...
      taxHistory,
      isDisputed
        ? "Outstanding levy contested while dispute is open."
        : "Land use charges current; no arrears in trailing 5 years.",
    ...
    [
      ...
      fraudAnalysis,
      isDisputed
        ? "Boundary overlap detected with neighbouring filing — escalated to bureau."
        : "No duplicate boundaries, no forged-stamp signals, signature cluster clean.",
  ...
  const score = Math.max(
    ...
    Math.min(
      100,
      factors.reduce((acc, f) => acc + f.weight * f.raw, 0),
    ),
  );
