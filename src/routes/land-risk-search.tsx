...
function LandRiskSearch() {
  ...
  async function search(value = query) {
    ...
    const selected = owned.find(
      (item) => item.id === value || item.passportId.toLowerCase() === value.trim().toLowerCase(),
    );
    if (!selected) {
    ...
    setRisk(
      typeof payload?.riskScore === "number"
        ? payload.riskScore
        : Math.round(
            getRiskIndicators(authoritative).reduce((sum, item) => sum + item.score, 0) / 6,
          ),
    );
  ...
  return (
    <AppShell
      title="Search Land Risk"
      subtitle="Check the verification and risk status of a property."
    ...
    >
      <div className="surface-card flex flex-col gap-3 p-5 sm:flex-row">
        <Input
          ...
          placeholder="Search by Passport ID or property ID"
        />
        ...
        <div className="surface-card mt-5 space-y-5 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="font-mono text-lg font-semibold">{property.passportId}</p>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <Metric
              label="Verification"
              value={
                verification === "manual_review" ? "Manual Review" : (verification ?? "Pending")
              }
            />
            <Metric label="Trust Score" value={`${property.trustScore} / 100`} />
            <Metric label="Risk Score" value={risk == null ? "Unavailable" : `${risk} / 100`} />
          </div>
