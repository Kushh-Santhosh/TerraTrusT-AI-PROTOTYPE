...
function QRGeneratorPage() {
  ...
  return (
    <AppShell
      ...
      subtitle="Create a scannable TerraTrust verification credential from a real passport URL."
    >
      <div className="grid gap-6 lg:grid-cols-[1fr_auto]">
        <div className="surface-card space-y-5 p-6">
          <div>
            <label className="text-sm font-medium" htmlFor="verification-url">
              Verification URL
            </label>
            <Input
              id="verification-url"
            ...
            />
            {!validUrl && (
              <p className="mt-2 text-xs text-destructive">Enter a valid HTTPS TerraTrust URL.</p>
            )}
          </div>
          <div>
            <label className="text-sm font-medium" htmlFor="passport-id">
              Property Passport ID <span className="text-muted-foreground">(optional)</span>
            </label>
            <Input
              id="passport-id"
              value={passportId}
              onChange={(event) => setPassportId(event.target.value)}
              placeholder="TT-KA-2609-P1R4N"
              className="mt-2 font-mono"
            />
          </div>
          <div>
            <label className="text-sm font-medium" htmlFor="qr-label">
              Label or title <span className="text-muted-foreground">(optional)</span>
            </label>
            <Input
              id="qr-label"
              value={label}
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={copyUrl} disabled={!validUrl}>
        ...
        <div className="surface-card flex min-w-0 flex-col items-center gap-4 p-6 text-center">
          ...
          <div className="rounded-xl bg-white p-4 ring-1 ring-border" ref={canvasRef}>
            {validUrl ? (
              ...
              <div className="grid h-[220px] w-[220px] place-items-center text-sm text-muted-foreground">
                Enter a valid URL
              </div>
            )}
            <div className="hidden">
              <QRCodeCanvas
                value={validUrl ? url : "https://example.invalid"}
                size={440}
                level="M"
                marginSize={16}
              />
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <Button size="sm" variant="outline" onClick={downloadPng} disabled={!validUrl}>
              <Download className="mr-1.5 h-4 w-4" /> PNG
            </Button>
            <Button size="sm" variant="outline" onClick={downloadSvg} disabled={!validUrl}>
