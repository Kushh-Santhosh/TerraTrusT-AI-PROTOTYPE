    ...
    >
      ...
      <div className="mt-6">
        <DataTable rows={rows} columns={[
          { key: "at", label: "Time", render: r => <span className="font-mono text-xs">{r.at}</span> },
          { key: "actor", label: "Actor", render: r => r.actor },
          { key: "lvl", label: "Level", render: r => <Pill tone={r.level === "alert" ? "danger" : r.level === "warning" ? "warning" : "info"}>{r.level}</Pill> },
          { key: "act", label: "Action", render: r => r.action },
          { key: "tgt", label: "Target", render: r => <span className="font-mono text-xs">{r.target}</span> },
        ]} />
      </div>
    </AppShell>
  );
