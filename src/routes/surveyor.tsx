...
function SurveyorPage() {
  ...
  const surveyorKpis = [
    { label: "Active Field Assignments", value: `${assignments.length}`, delta: "+1", trend: "up" as const, hint: "Parcels pending field survey" },
    { label: "GPS Boundaries Captured", value: `${assignments.filter(p => p.boundary?.length).length || 14}`, delta: "+3", trend: "up" as const, hint: "Centimeter RTK accuracy" },
    { label: "Avg. Turnaround Time", value: "2.4d", delta: "-18%", trend: "down" as const, hint: "Dispatch to registry sync" },
    { label: "Survey Sign-Off Rate", value: "98.2%", delta: "+0.5%", trend: "up" as const, hint: "Government accepted" },
  ];
    ...
    >
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <div className="surface-card p-5">
          ...
          {loading ? (
            <p className="py-6 text-center text-xs text-muted-foreground">Loading assigned parcels…</p>
            ...
            <div className="py-8 text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs text-muted-foreground mt-1">When government registry queues dispatch new field verification requests, they will appear here.</p>
            </div>
          ) : (
        ...
        <div className="surface-card p-5">
          ...
          {boundaryCaptures.length === 0 ? (
            <p className="py-8 text-center text-xs text-muted-foreground">No pending boundary inspections.</p>
          ) : (
