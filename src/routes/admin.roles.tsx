        ...
        {roles.map(r => (
          <div key={r.name} className="surface-card p-5">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary"><r.icon className="h-5 w-5" /></div>
              <div>
            ...
            <div className="mt-4 space-y-2">
              {r.perms.map(p => <div key={p} className="flex items-center gap-2 text-sm text-muted-foreground"><Pill tone="success">✓</Pill> {p}</div>)}
            </div>
          </div>
        ))}
      </div>
