...
const { data: property, error: propertyError } = await supabase.from("properties").insert({
  ...
  status: "pending",
  trust_score: 0,
}).select("id, passport_id, property_name, location, area, status, trust_score").single();
...
const { error: storageError } = await supabase.storage.from("property-documents").upload(storagePath, Buffer.from(`%PDF-1.4\n% TerraTrust QA evidence ${passportId}\n%%EOF\n`), { contentType: "application/pdf", upsert: false });
if (storageError) console.warn(`Storage upload unavailable for seed evidence: ${storageError.message}`);
const { data: document, error: documentError } = await supabase.from("property_documents").insert({
  ...
  verified: false,
}).select("id, name, kind, storage_path, verified, created_at").single();
if (documentError || !document) throw new Error(`Document insert failed: ${documentError?.message}`);
...
const status = verification.status ?? (verification.decision === "VERIFIED" ? "verified" : "manual_review");
const { error: verificationError } = await supabase.from("verification_results").insert({ property_id: property.id, provider: "n8n", result: verification });
if (verificationError) throw new Error(`Verification persistence failed: ${verificationError.message}`);
if (status === "manual_review") {
  const { data: reviewCase, error: caseError } = await supabase.from("review_cases").insert({ property_id: property.id, status: "open", reason: verification.decisionReason || "n8n manual review required" }).select("id, property_id, status, reason, created_at").single();
  if (caseError) throw new Error(`Review case persistence failed: ${caseError.message}`);
  console.log(JSON.stringify({ propertyId: property.id, passportId, documentId: document.id, reviewCaseId: reviewCase.id, n8nStatus: status, workflowId: verification.workflowId ?? null }, null, 2));
} else {
  console.log(JSON.stringify({ propertyId: property.id, passportId, documentId: document.id, reviewCaseId: null, n8nStatus: status, workflowId: verification.workflowId ?? null }, null, 2));
}
