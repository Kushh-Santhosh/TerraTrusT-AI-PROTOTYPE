...
async function runQATest() {
  ...
  const citizenNavAudit = await page.evaluate(() => {
    ...
    const navGroups = Array.from(document.querySelectorAll('aside nav > div > p')).map(p => p.innerText.trim());
    ...
    const forbiddenTerms = [
      'Government', 'Surveyor', 'Bank', 'Admin', 'Community',
      '/government', '/surveyor', '/bank', '/admin', '/community'
    ];
    const violations = navLinks.filter(l => 
      forbiddenTerms.some(term => l.href?.toLowerCase().includes(term.toLowerCase()) || l.text?.toLowerCase().includes(term.toLowerCase()))
    );
  ...
  await page.goto('http://localhost:3000/properties/p_001/verify', { waitUntil: 'domcontentloaded', timeout: 30000 });
  await new Promise(r => setTimeout(r, 3000));

  console.log('5. Clicking "Run Live Verification" in Chrome...');
  await page.evaluate(() => {
    const buttons = Array.from(document.querySelectorAll('button'));
    const runBtn = buttons.find(b => b.innerText.includes('Run Live Verification') || b.innerText.includes('Run'));
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const status = await page.evaluate(() => {
      ...
      return text.includes('WF-N8N-') || text.includes('All verification gates passed') || text.includes('VERIFIED');
    });
    if (status) {
      completedInUI = true;
  ...
  const uiData = await page.evaluate(() => {
    ...
    return {
      ...
      hasVerifiedPill: bodyText.includes('Digital Property Passport: Ready to issue') || bodyText.includes('verified'),
