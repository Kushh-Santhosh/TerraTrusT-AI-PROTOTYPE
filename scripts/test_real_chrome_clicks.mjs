...
async function runClickMatrix() {
  ...
  for (const item of rolesToTest) {
    ...
    for (const link of links) {
      ...
      const currentH1 = await page.$eval('h1', el => el.innerText.trim()).catch(() => 'NO_H1_FOUND');
      ...
      if (pass) {
        ...
        console.log(`  [PASS] Clicked "${link.label}" (${link.href}) -> URL: ${currentUrl} | H1: "${currentH1}"`);
      } else {
        summary[item.role].failed++;
        console.log(`  [FAIL] Clicked "${link.label}" (${link.href}) -> URL: ${currentUrl} | H1: "${currentH1}" | Error: ${hasError}`);
      }
