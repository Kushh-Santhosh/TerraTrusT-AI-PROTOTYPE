...
async function runFullQAMatrix() {
  ...
  const loginAudit = await page.evaluate(() => {
    ...
    const hasCommunity = text.toLowerCase().includes('community@') || text.toLowerCase().includes('verifier@');
  ...
  const citizenAudit = await page.evaluate(() => {
    ...
    const forbidden = navLinks.filter(l => 
      ['government', 'surveyor', 'bank', 'admin', 'community'].some(r => 
        l.href?.toLowerCase().includes(`/${r}`) || l.text?.toLowerCase().includes(r)
      )
    );
    return {
  ...
  await page.goto('http://localhost:3000/government', { waitUntil: 'domcontentloaded', timeout: 30000 });
  await new Promise(r => setTimeout(r, 1500));
  ...
  await page.goto('http://localhost:3000/community', { waitUntil: 'domcontentloaded', timeout: 30000 });
  await new Promise(r => setTimeout(r, 1500));
  const commAudit = await page.evaluate(() => {
    return {
      text: document.body.innerText.slice(0, 300),
      isDecommissioned: document.body.innerText.includes('Community Portal Unavailable') || document.body.innerText.includes('Portal Decommissioned')
    };
  });
  ...
  await page.goto('http://localhost:3000/properties/TT-8421-BLR/verify', { waitUntil: 'domcontentloaded', timeout: 30000 });
  ...
  if (n8nCaptures.length > 0) {
    ...
    console.log('n8n Response Confidence Score:', top.body?.confidenceScore || top.body?.confidence);
  }
