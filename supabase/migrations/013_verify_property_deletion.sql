-- Ensure owner-scoped deletion cannot report success when no row was removed.
...
begin
  ...
  values ('PROPERTY_DELETED', p_property_id::text, p_passport_id, 'citizen', 'Owner deleted property and associated records');
  ...
  if deleted_count <> 1 then
    raise exception 'Property deletion did not remove the expected row';
  end if;
