...
Show the server-side Gemini valuation result only when a fresh request visibly returns, labelled **AI-assisted indicative valuation**, alongside confidence explanation and the n8n verification result.
...
Do not present AI interpretation as legal title or official government valuation. Do not claim Gemini runs inside n8n until the canonical workflow visibly contains and executes its Gemini node.

## 3:00-3:45: Surveyor and Government

Show the review queue, shared boundary, document evidence, surveyor assignment surface, field evidence controls, and government decision surface using only a fresh authorized session.
...
Show a verified property, valuation, risk context, and a persisted collateral or loan assessment. Emphasize that bank users receive only information permitted by role and workflow state.
